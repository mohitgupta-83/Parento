...
const BRAND_DARK = "#1A0A2E";
const BRAND_MID = "#2D1265";
const BRAND_VIOLET = "#8B2FC9";
...
function AstroCheckoutModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const inp = `w-full px-4 py-3 rounded-xl border border-white/15 bg-white/8 text-white placeholder-white/40 focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/20 outline-none text-sm transition-all`;
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
          ...
          <motion.div
            ...
            style={{ background: `linear-gradient(135deg, #1A0A2E 0%, #2D1265 60%, #1A0A2E 100%)`, border: `1px solid #C9A84C30` }}>
            {/* Modal Header */}
            <div className="relative p-6 border-b border-white/10" style={{ background: `linear-gradient(135deg, #8B2FC920, #C9A84C15)` }}>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold tracking-wider" style={{ color: BRAND_GOLD }}>AstroJi</span>
              </div>
            ...
            <div className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
                ...
                <div className="flex gap-3">
                  {["Female", "Male", "Other"].map(g => (
                    <button key={g} onClick={() => setGender(g)} type="button"
                      className={`flex-1 py-2.5 rounded-xl text-sm font-bold border transition-all ${gender === g ? "text-[#1A0A2E]" : "text-white/60 hover:border-[#C9A84C]/50"}`}
              ...
              <button onClick={handlePay} disabled={loading}
                className="w-full py-4 rounded-2xl font-extrabold text-base text-[#1A0A2E] transition-all hover:scale-[1.02] active:scale-[0.98] shadow-xl disabled:opacity-70"
...
function AstroHeader() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-30 border-b border-white/10"
      style={{ background: `linear-gradient(90deg, #1A0A2E 0%, #2D1265 100%)`, backdropFilter: "blur(12px)" }}>
      <div className="mx-auto max-w-6xl px-4 sm:px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl border-2"
            style={{ background: `linear-gradient(135deg, #2D1265, #8B2FC9)`, borderColor: "#C9A84C50" }}>
            🔮
          </div>
          <div>
            <span className="text-xl font-black tracking-tight" style={{ color: "#C9A84C" }}>AstroJi</span>
            <div className="text-[10px] text-white/45 -mt-0.5">Spiritual Love Guidance</div>
          </div>
        </div>
        <button onClick={openCheckout}
          className="px-4 py-2 rounded-xl text-xs font-bold text-[#1A0A2E] transition-all hover:scale-105 shadow-md"
          style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>
          🔮 Get Reading
        </button>
      </div>
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ background: "#1A0A2E", borderTopColor: "#C9A84C" }}>
        <div className="px-4 py-1.5 flex items-center justify-between" style={{ background: "#0D0519" }}>
          <div className="flex items-center gap-1.5 text-xs font-bold text-white/60">
            <Clock className="w-3.5 h-3.5 animate-pulse" style={{ color: "#C9A84C" }} />
          ...
          <div className="flex items-center gap-1 px-3 py-1 rounded-lg font-mono font-black text-sm border"
            style={{ background: "#2D1265", color: "#C9A84C", borderColor: "#C9A84C30" }}>
        ...
        <div className="px-4 py-3 flex items-center justify-between gap-3 max-w-5xl mx-auto">
          <div>
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-black" style={{ color: "#C9A84C" }}>₹{product.price}</span>
          ...
          <button onClick={openCheckout}
            className="flex items-center gap-2 px-5 py-3 rounded-xl font-extrabold text-sm text-[#1A0A2E] transition-all hover:scale-105 active:scale-95 shadow-lg"
            style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>
...
function SoulmatePageContent() {
  ...
  return (
    <main className="min-h-screen pt-16 pb-32" style={{ background: "#0D0519", color: "#fff" }}>

      {/* HERO */}
      <section className="relative pt-8 pb-14 overflow-hidden"
        style={{ background: `linear-gradient(180deg, #1A0A2E 0%, #0D0519 100%)` }}>
        <StarsBg />
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
          <div className="flex justify-center mb-5">
            <div className="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border"
              style={{ background: "#8B2FC918", borderColor: "#8B2FC935", color: "#C9A84C" }}>
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-12 lg:gap-10 lg:items-center">
            ...
            <div className="lg:col-span-7">
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-tight mb-4 text-center lg:text-left">
                ...
                <span className="text-transparent bg-clip-text" style={{ backgroundImage: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>
                  Soulmate
                </span>
                {" "}❤️ Sketch
              ...
              <div className="lg:hidden mb-6 mx-auto max-w-xs">
                <div className="relative rounded-3xl overflow-hidden border-2 p-1 shadow-2xl"
                  style={{ borderColor: "#C9A84C35", background: `linear-gradient(135deg, #2D1265, #1A0A2E)` }}>
                  <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-black z-10 text-[#1A0A2E]"
                    style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>SPECIAL OFFER</div>
                  <img src={product.thumbnail} alt="Soulmate Sketch" className="w-full max-h-[260px] object-contain rounded-2xl" />
                  <div className="p-3 text-center">
                    <span className="text-2xl font-black" style={{ color: "#C9A84C" }}>₹{product.price}</span>
              ...
              <div className="flex flex-col items-center lg:items-start gap-3 mb-6">
                <button onClick={openCheckout}
                  className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 rounded-2xl text-base font-black text-[#1A0A2E] transition-all hover:scale-[1.03] active:scale-[0.97] shadow-2xl"
                  style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)`, boxShadow: "0 12px 40px rgba(201,168,76,0.4)" }}>
            ...
            <div className="hidden lg:flex lg:col-span-5 justify-center">
              <div className="relative rounded-3xl overflow-hidden border-2 p-2 shadow-2xl w-full"
                style={{ borderColor: "#C9A84C35", background: `linear-gradient(135deg, #2D1265, #1A0A2E)` }}>
                <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full text-xs font-black z-10 text-[#1A0A2E]"
                  style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>✨ SPECIAL OFFER</div>
      ...
      {/* TRUST IMAGE GALLERY */}
      <section className="py-12" style={{ background: "#120827" }}>
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="text-center mb-7">
            <span className="px-3 py-1 rounded-full text-xs font-bold border"
              style={{ color: "#C9A84C", borderColor: "#C9A84C35", background: "#C9A84C12" }}>
              ✨ As Seen &amp; Trusted
            </span>
      ...
      {/* WHAT YOU GET */}
      <section className="py-14" style={{ background: "#0D0519" }}>
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="text-center mb-10">
            <span className="px-3 py-1 rounded-full text-xs font-bold border"
              style={{ color: "#C9A84C", borderColor: "#C9A84C35", background: "#C9A84C12" }}>🎁 Complete Package</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {whatYouGet.map((item, i) => (
              <motion.div key={i}
                ...
                className="p-5 rounded-2xl border border-white/10 hover:border-[#C9A84C]/35 transition-all"
                style={{ background: `linear-gradient(135deg, #2D126518, #8B2FC910)` }}>
                <div className="text-3xl mb-3">{item.icon}</div>
                <h3 className="text-sm font-extrabold mb-1" style={{ color: "#C9A84C" }}>{item.title}</h3>
          ...
          <div className="mt-10 text-center">
            <button onClick={openCheckout}
              className="px-10 py-4 rounded-2xl font-extrabold text-base text-[#1A0A2E] transition-all hover:scale-105 shadow-xl"
              style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)` }}>
      ...
      {/* ABOUT DR. SHALINI */}
      <section className="py-14" style={{ background: "#120827" }}>
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          <div className="rounded-3xl border border-white/10 p-8"
            style={{ background: `linear-gradient(135deg, #2D126528, #1A0A2E)` }}>
            <div className="text-center mb-6">
              <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 text-4xl border-2"
                style={{ background: "#2D1265", borderColor: "#C9A84C45" }}>🌟</div>
              <div className="inline-block px-3 py-1 rounded-full text-xs font-bold mb-3 border"
                style={{ color: "#C9A84C", borderColor: "#C9A84C35", background: "#C9A84C12" }}>About Me</div>
            ...
            <div className="space-y-3 text-sm text-white/55 leading-relaxed">
              ...
              <p className="text-center pt-2 font-medium" style={{ color: "#C9A84C" }}>Thank you for allowing me to be part of your journey. ❤️</p>
            </div>
      ...
      <section className="py-14" style={{ background: "#120827" }}>
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="text-center mb-10">
            <span className="px-3 py-1 rounded-full text-xs font-bold border"
              style={{ color: "#C9A84C", borderColor: "#C9A84C35", background: "#C9A84C12" }}>⭐ Testimonials</span>
      ...
      <section className="py-14" style={{ background: "#0D0519" }}>
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          <div className="text-center mb-10">
            <span className="px-3 py-1 rounded-full text-xs font-bold border"
              style={{ color: "#C9A84C", borderColor: "#C9A84C35", background: "#C9A84C12" }}>❓ FAQ</span>
            ...
            {faqs.map((faq, i) => (
              <div key={i} className="rounded-2xl border border-white/10 overflow-hidden"
                ...
                <button className="w-full flex items-center justify-between p-5 text-left hover:bg-white/5 transition-colors"
                  ...
                  <span className="text-lg flex-shrink-0" style={{ color: "#C9A84C" }}>{openFaq === i ? "−" : "+"}</span>
                </button>
                <AnimatePresence>
                  {openFaq === i && (
      ...
      <section className="py-16 text-center relative overflow-hidden"
        style={{ background: `linear-gradient(135deg, #1A0A2E, #2D1265, #1A0A2E)` }}>
        <StarsBg />
        <div className="relative mx-auto max-w-xl px-4">
          ...
          <div className="inline-block rounded-3xl border-2 p-8 w-full"
            style={{ borderColor: "#C9A84C35", background: "#0D051978" }}>
            <div className="flex items-baseline justify-center gap-3 mb-2">
              <span className="text-5xl font-black" style={{ color: "#C9A84C" }}>₹{product.price}</span>
            ...
            <button onClick={openCheckout}
              className="mt-6 w-full py-4 rounded-2xl font-extrabold text-lg text-[#1A0A2E] transition-all hover:scale-[1.03] active:scale-[0.97] shadow-2xl"
              style={{ background: `linear-gradient(135deg, #C9A84C, #F0C674)`, boxShadow: "0 15px 50px rgba(201,168,76,0.35)" }}>
      ...
      {/* FOOTER */}
      <footer className="py-8 border-t border-white/10 text-center" style={{ background: "#0D0519" }}>
        <div className="mx-auto max-w-5xl px-4">
          <div className="flex items-center justify-center gap-2 mb-3">
            <span className="text-xl font-black" style={{ color: "#C9A84C" }}>AstroJi</span>
